...
class Object:
    ...
    def move(self, direction):
        ...
        if (self.can_turn == [True, False] or self.can_turn == [False, True]) and self.frame_lim == 2:
            if get_map_letter(*self.map_pos) in PATH:
                self.pos += pygame.Vector2(self.dir[0] * RATIO[0] / self.speed, self.dir[1] * RATIO[1] / self.speed)
        else:
            if get_map_letter((self.map_pos.x + direction[0]) % len(map[0]), self.map_pos.y + direction[1]) in PATH and \
